...
Page({
  ...
  onLoad: function(options) {
    ...
    this.setData({
      ...
      ['replyObj.type']: wx.getStorageSync('questionOrPostil')=='question'?0:1, //0提问 1批注
    ...
    if (options.from11 == 'draw') {
      console.log(options);
      this.setData({
        ['replyObj.value']: options.url,
      ...
      let obj = {
        pageNum: this.data.replyObj.pageNum,
        ruserLessonQaId: this.data.replyObj.ruserLessonQaId,
        snum: this.data.replyObj.snum
      };
      console.log(obj)
      wx.redirectTo({
        url: '/pages/dowork/draw?comefrom=commit&' + parseParam(obj),
      })
    }
  },
