...
Page({
  ...
  onLoad: function(options) {
    ...
    this.setData({
      ...
      getDataObj: {
        ...
        ruserId: options.ruserId ? options.ruserId : wx.getStorageSync('ruserId'), //学生ID
        page: 1, //当前页索引值
        size: 10000, //显示大小
        // stat: options.stat
      }
    })
    console.log(this.data.getDataObj);
  },
  ...
  onShow() {
    ...
    if(!!wx.getStorageSync('statSourceTypeAnnotationStat')){
      this.setData({
        ['getDataObj.stat']: Number(wx.getStorageSync('statSourceTypeAnnotationStat'))
      });
    }
    wx.setStorageSync('statSourceTypeAnnotationStat', '')
  ...
  toggle() {
    ...
    wx.setStorageSync('commitIndex', this.data.info.index)
    this.setData({
