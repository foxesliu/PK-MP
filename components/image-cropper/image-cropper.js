...
Component({
  ...
  methods: {
    clik(){
      this.triggerEvent('changeCropper', true)
      // console.log(this.properties.isCropper)
    },
    /**
     * 上传图片
     */
      ...
      this._draw(()=>{
        wx.canvasToTempFilePath({
          ...
          success: (res) => {
            ...
            // });
            wx.uploadFile({
              url: app.globalData.apiPrefix + '/ruser/ruser/v1/upload/pic',
              ...
              success(res){
                _this.triggerEvent('changeCropper', true);
                _this.triggerEvent('changeHead', JSON.parse(res.data).data)
                wx.setStorageSync('pic', JSON.parse(res.data).data)
